...
pub const RenderingContext2D = struct {
            ...
            switch (commands.get(cmdIdx)) {
                ...
                @intFromEnum(PathCommand.lineTo) => {
                    ...
                    if (idk == 0) {
                        ...
                        switch (self.canvas.renderer) {
                            .Cairo => {
                                cairoBackend.renderTriangle(
                                    currX, currY, curr2X, curr2Y, x, y, 
                                    self._cairoItems, self.fillStyle
                                );
                            },
                            .Software => {
                                softBackend.renderTriangle(
                                    currX, currY, curr2X, curr2Y, x, y, 
                                    self._softItems, self.fillStyle
                                );
                            },
                        }
                        currX = x;
                ...
                @intFromEnum(PathCommand.arc) => {
                    switch (self.canvas.renderer) {
                        .Cairo => {
                            cairoBackend.renderEllipse(
                                args.get(argIdx), args.get(argIdx+1), args.get(argIdx+2), args.get(argIdx+2),
                                self._cairoItems, self.fillStyle
                            );
                        },
                        .Software => {
                            softBackend.renderEllipse(
                                args.get(argIdx), args.get(argIdx+1), args.get(argIdx+2), args.get(argIdx+2),
                                self._softItems, self.fillStyle
                            );
                        },
        ...
        while (cmdIdx < commands.len) : (cmdIdx += 1) {
            switch (commands.get(cmdIdx)) {
                ...
                @intFromEnum(PathCommand.lineTo) => {
                    ...
                    switch (self.canvas.renderer) {
                        .Cairo => {
                            cairoBackend.renderLine(
                                currX, currY, x, y, self.lineWidth, self.lineCap[0],
                                self._cairoItems, self.strokeStyle, self.imageSmoothingEnabled
                            );
                        },
                        .Software => {
                            softBackend.renderLine(
                                currX, currY, x, y, self.lineWidth, self.lineCap[0],
                                self._softItems, self.strokeStyle, self.imageSmoothingEnabled
                            );
                        },
                    }
                    currX = x;
                ...
                @intFromEnum(PathCommand.arc) => {
                    switch (self.canvas.renderer) {
                        .Cairo => {
                            cairoBackend.renderStrokeEllipse(
                                args.get(argIdx), args.get(argIdx+1), args.get(argIdx+2), args.get(argIdx+2), self.lineWidth,
                                self._cairoItems, self.strokeStyle, self.imageSmoothingEnabled
                            );
                        },
                        .Software => {
                            softBackend.renderStrokeEllipse(
                                args.get(argIdx), args.get(argIdx+1), args.get(argIdx+2), args.get(argIdx+2), self.lineWidth,
                                self._softItems, self.strokeStyle, self.imageSmoothingEnabled
                            );
                        },
    ...
    pub fn strokeRect(self: *RenderingContext2D, x: f32, y: f32, w: f32, h: f32) void {
        switch (self.canvas.renderer) {
            ...
            .Software => {
                softBackend.renderStrokeRectangle(
                    x, y, w, h, self.lineWidth,
                    self._softItems, self.strokeStyle, self.imageSmoothingEnabled
                );
            },
